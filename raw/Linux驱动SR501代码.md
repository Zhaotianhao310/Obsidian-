...
ssize_t sr501_drv_read(struct file *file, char __user *buffer, size_t size, loff_t *offset)
{
    return 0;
}

unsigned int sr501_drv_poll (struct file *file, struct poll_table_struct *wait)
{
    return 0;
}
...
/* 定义自己的file_operations结构体  文件操作接口映射表*/
...
ssize_t sr501_read (struct file *file, char __user *buf, size_t size,  loff_t *offset)
{
    int val;
...
static unsigned int sr501_poll(struct file *fp, poll_table * wait)
{
...
static int sr501_detect(void *argc)
{
    int val = -1;
    int pre = -1;
    while(1)
...
static int sr501_probe(struct platform_device *pdev)
{
    ...
    device_create(sr501_class, NULL, MKDEV(major,0), NULL, "/dev/sr501"); 
    	/* 3. 创建内核线程检测GPIO引脚 */
	kernel_thread(sr501_detect, NULL, 0);
    return 0;
}
